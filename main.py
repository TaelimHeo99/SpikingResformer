...
def parse_args():
    ...
    parser.add_argument(
        "--wandb-project",
        type=str,
        default="SpikingResformer",
        help="Weights & Biases project name",
    )
    parser.add_argument(
        "--wandb-entity", type=str, help="Weights & Biases entity or team name"
    )
...
def setup_wandb(args, logger: logging.Logger):
    if not args.wandb or not is_main_process():
        return None
    if wandb is None:
        raise ImportError(
            "wandb is not installed. Run `pip install wandb` in your environment."
        )
    ...
    logger.info("W&B run initialized: {}".format(run.name))
    return run
...
def load_data(
    ...
    if dataset_type == "CIFAR10":
        ...
        dataset = CIFAR10DVS(
            dataset_dir, data_type="frame", frames_number=T, split_by="number"
        )
        dataset_train, dataset_test = (
            DatasetSplitter(dataset, 0.9, True),
            DatasetSplitter(dataset, 0.1, False),
        )
...
def test(
    ...
    with torch.no_grad():
        ...
        for idx, (image, target) in enumerate(data_loader_test):
            ...
            if (
                args.print_freq != 0
                and ((idx + 1) % int(len(data_loader_test) / args.print_freq)) == 0
            ):
                logger.debug("Test: [{}/{}]".format(idx + 1, len(data_loader_test)))
        logger.info(
            "Throughput: {:.5f} it/s".format(
    ...
    ops, params = profile(
        model,
        inputs=(torch.rand(input_size).cuda().unsqueeze(0),),
        verbose=False,
        custom_ops={
            ...
            wandb_run.log(
                {
                    ...
                    "test/acc1": test_acc1,
                    "test/acc5": test_acc5,
                    "best/test_acc1": max(max_acc1, test_acc1),
                },
                step=epoch,
            )
